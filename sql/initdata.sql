...
SET @community_answer_status_exists = (
    SELECT COUNT(*)
    FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'community_answer'
      AND COLUMN_NAME = 'status'
);
SET @community_answer_status_sql = IF(
    @community_answer_status_exists = 0,
    'ALTER TABLE community_answer ADD COLUMN status VARCHAR(32) NOT NULL DEFAULT ''approved'' COMMENT ''pending / approved / rejected'' AFTER content',
    'SELECT 1'
);
PREPARE community_answer_status_stmt FROM @community_answer_status_sql;
EXECUTE community_answer_status_stmt;
DEALLOCATE PREPARE community_answer_status_stmt;
