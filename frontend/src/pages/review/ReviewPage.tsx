    ...
    >
      <DataTableCard
        ...
        columns={[
          ...
          { title: '类型', render: (_, row) => <Tag>{pickText(row, ['contentType', 'type'])}</Tag> },
          { title: '状态', render: (_, row) => <StatusTag value={pickText(row, ['status'])} /> },
          { title: '评分', render: (_, row) => <Rate allowHalf defaultValue={Number(row.score || 0)} onChange={(score) => rate.mutate({ id: String(row.id || row.reviewItemId), score })} /> },
