...
const metrics = computed(() => [
  ...
  },
  {
    ...
    desc: overview.value.wrongCount > 0 ? '低分题可在错题本中继续处理。' : '当前没有待处理题目。'
  },
  {
    label: '计划完成率',
    value: `${overview.value.planCompletionRate}%`,
    desc: overview.value.planCompletionRate > 0 ? '显示当前计划完成进度。' : '生成计划后会显示进度。'
  }
])
...
const primaryMission = computed(() => {
  ...
  if ((overview.value.planHealthScore ?? 100) < 70) {
    return {
      to: '/plan',
      title: '调整当前学习计划',
      description: `当前计划健康分为 ${overview.value.planHealthScore ?? 0}。任务节奏偏紧时，建议先调整任务安排。`,
...
const todaySignals = computed(() => [
  ...
  },
  {
    ...
    detail: (overview.value.planHealthScore ?? 100) >= 70 ? '当前任务节奏基本合理。' : '建议调整任务量或重排重点。' ,
    action: '调整计划',
    to: '/plan',
...
const quickActions = [
  ...
  { to: '/wrong', label: '错题本', title: overview.value.wrongCount > 0 ? `处理 ${overview.value.wrongCount} 道错题` : '查看错题与复习记录', badge: '修复', desc: '处理低分题，减少同类错误重复出现。' },
