...
import { App as AntApp, Button, Form, Input, Modal, Popconfirm, Select, Space } from 'antd'
  ...
  return (
    <ModulePage
      ...
      metrics={[
        ...
        { label: '面试中', value: rows.filter((row) => pickText(row, ['status']).includes('interview')).length, hint: '重点跟进' },
        { label: '待分析', value: rows.filter((row) => !row.analysis).length, hint: '可刷新分析' },
        { label: '状态', value: query.isFetching ? '刷新中' : '已同步', hint: '看板数据' }
      ]}
      actions={<Space wrap><Input.Search allowClear placeholder="搜索公司或岗位" onSearch={(keyword) => setFilters((current) => ({ ...current, keyword }))} /><Select allowClear placeholder="状态" style={{ width: 130 }} options={[{ value: 'APPLIED', label: '已投递' }, { value: 'INTERVIEW', label: '面试中' }, { value: 'OFFER', label: 'Offer' }, { value: 'REJECTED', label: '已拒绝' }]} onChange={(status) => setFilters((current) => ({ ...current, status }))} /><Button type="primary" onClick={() => setOpen(true)}>新增投递</Button></Space>}
    >
      <DataTableCard
        title="投递列表"
        data={query.data}
        ...
        columns={[
          ...
          { title: '操作', render: (_, row) => <Space><Select size="small" style={{ width: 120 }} defaultValue={pickText(row, ['status'], 'APPLIED')} options={[{ value: 'APPLIED', label: '已投递' }, { value: 'INTERVIEW', label: '面试中' }, { value: 'OFFER', label: 'Offer' }, { value: 'REJECTED', label: '已拒绝' }]} onChange={(status) => updateStatus.mutate({ id: String(row.id || row.applicationId), status })} /><Popconfirm title="刷新投递分析" description="系统会重新生成匹配度和行动建议。" onConfirm={() => refresh.mutate(String(row.id || row.applicationId))}><Button size="small" loading={refresh.isPending}>刷新分析</Button></Popconfirm></Space> }
        ]}
      />
