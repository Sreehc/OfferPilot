...
export function ApplicationBoardPage() {
  ...
  const query = useQuery({ queryKey: ['applications'], queryFn: () => fetchApplicationBoardApi().then((response) => response.data) })
  ...
  return (
    <ModulePage
      ...
      actions={<Space><Button type="primary" onClick={() => setOpen(true)}>新增投递</Button></Space>}
    >
      <DataTableCard
        title="投递列表"
        data={query.data}
