...
public class JwtSecretValidator implements CommandLineRunner {
    ...
    private static final String DEFAULT_SECRET = "ByteCoachJwtSecretByteCoachJwtSecret123456";
    ...
    public void run(String... args) {
        if (jwtSecret == null || jwtSecret.isBlank() || jwtSecret.equals(DEFAULT_SECRET)) {
            throw new IllegalStateException(
                    "SECURITY: bytecoach.security.jwt-secret is set to the default value or is blank. "
                            + "You MUST configure a strong, unique secret for production deployment. "
                            + "Application startup aborted.");
        }
        if (jwtSecret.length() < 32) {
            throw new IllegalStateException(
                    "SECURITY: bytecoach.security.jwt-secret is too short (minimum 32 characters). "
                            + "Application startup aborted.");
        }
