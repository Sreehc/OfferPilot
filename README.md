...
- **知识库**：内置种子资料导入、文档切分、关键词检索
- **管理后台**：分类、题库、知识文档 CRUD（ADMIN 角色）

## 技术栈

- **后端**：Spring Boot 3、Java 17、MyBatis-Plus、MySQL 8、Redis 7、Spring Security、JWT、Knife4j
- **前端**：Vue 3、Vite、TypeScript、Pinia、Vue Router、Axios、Element Plus、Tailwind CSS、ECharts
- **AI**：OpenAI-compatible 网关抽象，支持任意兼容 API
...
- **RAG 检索**：DB 级 LIKE 预过滤 + 内存关键词评分，兼顾性能与精度
